import { CQLQuery } from "../types/search";
import { CQL_PATTERNS } from "../constants/search";
...
export class CQLParser {
  ...
  static parseCQL(query: string): CQLQuery {
    const isCQL = this.isCQLSyntax(query);
    ...
    const parsed = this.extractCQLElements(query);
   ...
   * 提取 CQL 查询中的元素
   */
  private static extractCQLElements(query: string) {
    const fields: string[] = [];
   ...
   * 验证 CQL 语法是否正确
   */
  static validateCQL(query: string): { isValid: boolean; error?: string } {
    if (!this.isCQLSyntax(query)) {
